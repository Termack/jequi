...
#[no_mangle]
pub extern "C" fn set_response_header(resp: *mut Response, header: *const c_char, value: *const c_char) {
...
pub extern "C" fn write_response_body(resp: *mut Response, string: *const c_char){
    let resp = get_object_from_pointer(resp);
    resp.write_body(unsafe { CStr::from_ptr(string) }.to_bytes()).unwrap();
}
...
pub extern "C" fn get_request_body(req: *mut Request) -> *const c_char {
    let req = get_object_from_pointer(req);
    CString::new(req.get_body().unwrap_or(&"".to_string()).as_str()).unwrap().into_raw()
}
